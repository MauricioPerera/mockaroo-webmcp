...
const DATA_FILE = path.join(__dirname, '..', 'data', 'schemas.json');
...
function initStorage() {
  if (!fs.existsSync(DATA_FILE)) {
    fs.writeFileSync(DATA_FILE, JSON.stringify(DEFAULT_PRESETS, null, 2), 'utf8');
  }
}
...
function saveSchema(id, schemaData) {
  ...
  fs.writeFileSync(DATA_FILE, JSON.stringify(schemas, null, 2), 'utf8');
  return schemas[id];
}
